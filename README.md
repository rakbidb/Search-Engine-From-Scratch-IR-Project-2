...
I added a SPIMI-style indexing mode as an alternative to BSBI. In SPIMI, we don't build the full `td_pairs` list. Instead, each block is parsed once and we directly build an in‑memory dictionary of postings and TFs, then flush it to disk.

Implementation:
- `bsbi.py` adds `invert_write_spimi(...)` and `index_spimi(...)`.
- `index_spimi(...)` mirrors the BSBI pipeline but avoids the `td_pairs` intermediate list.

Usage idea:
- Call `BSBIIndex(...).index_spimi()` to build the index using SPIMI mode.
...
## 2) Trie-Based Term Dictionary
I added an optional Trie-based dictionary to store term -> id mappings, which can be more efficient for prefix‑based storage and lookups.

Implementation:
- `util.py` adds `TrieNode` and `TrieIdMap`.
- `BSBIIndex` now accepts `use_trie=True` to switch the term dictionary from `IdMap` (hash table) to `TrieIdMap`.

Usage idea:
- `BSBIIndex(..., use_trie=True)` to build index with a Trie term dictionary.
